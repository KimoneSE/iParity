...
<script>
	...
	function getComment(){
		$.ajax({
            type: "GET",
            url: "http://localhost:8080/iParity/admin/comment",
            
            success: function(data) {
                $("#myContent").empty();
                var list=data.list;
				if(list==null||list.length==0){
					var tip = '<div>暂时没有敏感评论</div>'
					$("#myContent").append(tip);
				}else{
					for(var i=0;i<list.length;i++){
						var delUrl = "http://localhost:8080/iParity/admin/delComment?id="+list[i].id;
						var modUrl = "http://localhost:8080/iParity/admin/modComment?id="+list[i].id;
						var panel = '<div class="panel "><div class="row"><div class="col-xs-2 headLabel">'+list[i].uid+'</div>'+
						'<div class="col-xs-6 headLabel">'+list[i].content+'</div><div class="col-xs-2 headLabel"><a href="'+delUrl+'">删除</a></div>'+
